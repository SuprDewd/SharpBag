...
namespace SharpBag.Math.ForInt32
{
	...
	public class Matrix : MatrixBase<int, Matrix>
	{
		#region Accessors
		...
		public abstract class Accessor : IEnumerable<Vector>
		{
			...
			/// <summary>
			/// Gets or sets the <see cref="SharpBag.Math.ForInt32.Vector"/> at the specified index.
			/// </summary>
		...
		public override int Determinant
		{
			get
			{
				...
				{
					int minorDet = Matrix.Minor(this, 0, j).Determinant;
		...
		public override Matrix Transpose
		{
			get
			{
				...
				MatrixBase<int, Matrix>.InternalTranspose(result, this);
				return result;
			}
		}
		...
		public Matrix(int[,] elements) : base(elements) { }
		...
		public static Matrix Identity(int size)
		{
			...
			MatrixBase<int, Matrix>.InternalIdentity(result, size, 1);
			return result;
		}
		...
		public static Matrix Diagonal(params int[] diagonals)
		{
			Matrix result = new Matrix(diagonals.Length, diagonals.Length);
			MatrixBase<int, Matrix>.InternalDiagonal(result, diagonals);
			return result;
		}
		...
		/// The result of the operator.
		/// </returns>
		public static Matrix operator *(Matrix left, int right) { return Matrix.Multiply(left, right); }
		...
		/// The result of the operator.
		/// </returns>
		public static Matrix operator *(int left, Matrix right) { return Matrix.Multiply(left, right); }
		...
		/// <param name="right">The right matrix.</param>
		/// <returns></returns>
		public static Matrix Multiply(int left, Matrix right) { return Matrix.Multiply(right, left); }
		...
		public static Matrix Minor(Matrix matrix, int row, int column)
		{
			...
			MatrixBase<int, Matrix>.InternalMinor(result, matrix, row, column);
			return result;
		}
		...
		public static Matrix EchelonForm(Matrix matrix)
		{
			...
			for (int i = 0; i < matrix.RowCount; i++)
			{
				if (result[i, i] == 0)
				{
					...
					{
						if (result[j, i] != 0) MatrixBase<int, Matrix>.SwapRows(result, i, j);
					}
				}
				...
				if (result[i, i] != 1)
				{
					for (int j = i + 1; j < result.RowCount; j++)
					{
						if (result[j, i] == 1) MatrixBase<int, Matrix>.SwapRows(result, i, j);
					}
				}
		...
		public static Matrix Adjoint(Matrix matrix)
		{
			...
			for (int i = 0; i < matrix.RowCount; i++)
			{
				...
				{
					result[i, j] = (int)Math.Pow(-1, i + j) * Matrix.Minor(matrix, i, j).Determinant;
				}
			}
		...
		public static Matrix Augment(Matrix left, Matrix right)
		{
			...
			MatrixBase<int, Matrix>.InternalAugment(result, left, right);
			return result;
		}
		...
		/// <summary>
		/// Performs an implicit conversion from an array of numbers to <see cref="SharpBag.Math.ForInt32.Matrix"/>.
		/// </summary>
		/// <param name="elements">The elements.</param>
		/// <returns>
		/// The result of the conversion.
		/// </returns>
		public static implicit operator Matrix(int[,] elements) { return new Matrix(elements); }

		/// <summary>
		/// Performs an implicit conversion from <see cref="SharpBag.Math.ForInt32.Matrix"/> to an array of numbers.
		/// </summary>
		/// <param name="matrix">The matrix.</param>
		/// <returns>
		/// The result of the conversion.
		/// </returns>
		public static implicit operator int[,](Matrix matrix) { return matrix.Elements; }

		/// <summary>
		/// Performs an explicit conversion from <see cref="SharpBag.Math.ForInt32.Matrix"/> to <see cref="SharpBag.Math.ForInt32.Vector"/>.
		/// </summary>
		/// <param name="matrix">The matrix.</param>
		/// <returns>
		/// The result of the conversion.
		...
		public override bool Equals(Matrix other) { return MatrixBase<int, Matrix>.InternalEquals(this, other); }
